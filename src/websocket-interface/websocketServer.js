...
export const createWebsocketServer = async (port, allowedHosts) => {
  const httpServer = await createHttpServer(port, allowedHosts) // promise will reject if can't start HTTP server on specified port
